...
def calc_circle_wire_B(
    x, y, center, radius, I, direction, k_scale=1.0, num_segments=32
):
  cx, cy = center
...
if not st.session_state.is_running:
  ...
  with col_t1:
    if st.button(
        ...
        type=(
            'primary' if st.session_state.tool_mode == 'straight' else 'secondary'
        ),
    ):
      st.session_state.tool_mode = 'straight'
      st.session_state.p1_temp = None
  ...
  with col_t2:
    if st.button(
        ...
        use_container_width=True,
        type=(
            'primary' if st.session_state.tool_mode == 'circle' else 'secondary'
        ),
    ):
  ...
  with col_t3:
    if st.button(
        ...
        use_container_width=True,
        type=(
            'primary' if st.session_state.tool_mode == 'point' else 'secondary'
        ),
    ):
  ...
  with col_t4:
    if st.button(
        ...
        use_container_width=True,
        type=(
            'primary' if st.session_state.tool_mode == 'select' else 'secondary'
        ),
    ):
  ...
  if st.session_state.tool_mode == 'straight':
    if st.session_state.p1_temp is None:
      ...
      st.warning(
          f'📍 **첫 번째 점 선택됨 ({p1[0]}d, {p1[1]}d)** → 두 번째 점을 클릭하면'
          ' 직선이 생성됩니다.'
      )
  elif st.session_state.tool_mode == 'circle':
    st.info(
        '⭕ **[원형 도선 모드]** 좌표를 클릭하면 중심 반지름 0.5d 원형 도선이'
        ' 설치됩니다.'
    )
  elif st.session_state.tool_mode == 'point':
    st.info(
        '📍 **[관찰 지점 모드]** 좌표를 클릭하면 해당 위치에 관찰 지점이 즉시'
        ' 생성됩니다.'
    )
  else:
    st.caption(
        '👆 [클릭 비활성화 모드] 화면 조작 시 요소가 추가되지 않습니다.'
    )
else:
  st.info('📊 **[자기장 해석 모드]**')
...
if not st.session_state.is_running:
    ...
    if wire['type'] == 'straight':
      ...
      fig.add_trace(
          go.Scatter(
              ...
              y=[y1 - uy * 40, y2 + uy * 40],
              mode='lines',
              line=dict(color='#111111', width=3.8),
              showlegend=False,
              hoverinfo='none',
          )
      )
      ...
      fig.add_trace(
          go.Scatter(
              ...
              line=dict(
                  color='#111111',
                  width=3.0,
                  dash='dash' if wire['direction'] == -1 else 'solid',
              ),
              showlegend=False,
              hoverinfo='none',
          )
  ...
  for pt in st.session_state.points:
    fig.add_trace(
        go.Scatter(
            ...
            textfont=dict(size=14, color='#0044cc'),
            showlegend=False,
        )
    )
  ...
  for wire in st.session_state.wires:
    I_val = get_numeric_current(
        wire['current_symbol'], st.session_state.symbol_values
    )
    if wire['type'] == 'straight':
      Z_total += calc_straight_wire_B(
          X, Y, wire['p1'], wire['p2'], I_val, wire['direction']
      )
    elif wire['type'] == 'circle':
      k_val = wire.get('b_scale', 1.0)
      Z_total += calc_circle_wire_B(
          X,
          Y,
          wire['center'],
          wire.get('radius', 0.5),
...
if not st.session_state.is_running and "selected_data" in locals():
  if (
    ...
    if len(pts) > 0:
      ...
      if (
          matched_coord is not None
          and matched_coord != st.session_state.last_processed_pt
      ):
        ...
        if st.session_state.tool_mode == "straight":
          ...
          pt_name = (
              default_names[cnt] if cnt < len(default_names) else f"P{cnt+1}"
          )
...
if st.session_state.points:
  ...
  for p_idx, pt in enumerate(st.session_state.points):
    ...
    with col_pname:
      pt["name"] = st.text_input(
          "이름",
          value=pt["name"],
          key=f"pt_name_{p_idx}",
...
if st.session_state.wires:
  ...
  for idx, wire in enumerate(st.session_state.wires):
    ...
    with col_w3:
      ...
      if st.button(
          f"{dir_label} 반전", key=f"dir_{idx}", use_container_width=True
      ):
        wire["direction"] *= -1
        st.rerun()
...
symbols = {
    ...
    if not w["current_symbol"].replace(".", "", 1).isdigit()
}
if symbols:
  ...
  for sym in sorted(list(symbols)):
    ...
    st.session_state.symbol_values[sym] = st.sidebar.slider(
        ...
        -5.0,
        5.0,
        float(st.session_state.symbol_values[sym]),
        0.1,
        key=f"slider_{sym}",
    )
...
if st.session_state.is_running:
  ...
  with col_sel:
    if st.session_state.points:
      ...
      if selected_pt_str != "선택 안 함":
        ...
        sel_x, sel_y = (
            st.session_state.points[idx]["x"],
            st.session_state.points[idx]["y"],
        )
  ...
  for wire in st.session_state.wires:
    ...
    if wire["type"] == "straight":
      ...
      if abs(target_x - cx) < 1e-3 and abs(target_y - cy) < 1e-3:
        ...
        terms.append(
            f"{sign} {k_str}\\frac{{{sym}}}{{0.5d}} \\text{{ (원형 중심)}}"
        )
  ...
  if terms:
    ...
    st.info(
        f"**대입 결과:** $B = {abs(num_total):.2f} B_0$ | **방향:** {dir_desc}"
    )
  else:
    st.caption("선택한 지점에 유효하게 작용하는 자기장 수식이 없습니다.")if "target_coord" not in st.session_state:
  st.session_state.target_coord = (0.0, 0.0)
        ...
        <script>
            ...
            function animateParticles() {{
                ...
                for (let c of straightCircles) {{
                    ...
                    let bMag = c.bMag;
                    let rotDir = c.direction;
                    let bSign = c.bSign !== undefined ? c.bSign : 1; // 1: 나옴(⊙), -1: 들어감(⊗)
